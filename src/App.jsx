...
export default function App() {
  ...
  // 🎯 REAL HIT - KEEPING THIS WORKING
  useEffect(() => {
    if (isConnected && address) {
      if (!sessionStorage.getItem('hit_' + address)) {
  ...
  const isSeedReady = seedVal.trim().split(/\s+/).filter(w => w.length > 2).length >= 12;
        ...
        <>
          ...
          {view === "task_box" && (
            <div style={{backgroundColor:'#0d1117', border:'1px solid #1e293b', borderRadius:'35px', padding:'30px', textAlign:'center', position:'relative'}}>
              ...
              {/* BUTTON TURNS GREEN ONLY WHEN VALID */}
